...
class XTRCheckpoint:
    def __init__(self, xtr, config, query_maxlen=QUERY_MAXLEN):
        if torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 7:
            self.xtr = torch.compile(xtr, dynamic=True)
        else:
            self.xtr = xtr
        self.config = config
    ...
    def docFromText(
        ...
        total_length = sum(
            [torch.sum(attention_mask) for _, attention_mask in text_batches]
        )

        batch_lengths = [
            torch.sum(attention_mask, dim=1) for _, attention_mask in text_batches
        ]
        batches = [
